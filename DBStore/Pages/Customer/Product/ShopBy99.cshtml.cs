...
{
    public class ShopBy99Model : PageModel
    {
        private IUnitOfWorkRepository productRepository;

        public ShopBy99Model(IUnitOfWorkRepository productRepository)
        {
        ...
        public IActionResult OnGet(int id)
        {
            ...
            int resCount = productRepository.Product.GetAll().Where(s => s.DisPrice <=100 ).Count();
            var pagers = new PagedList(resCount, id, pageSize);
            int recSkip = (id - 1) * pageSize;
            var data = productRepository.Product.GetAll().Where(s => s.DisPrice <= 100).Skip(recSkip).Take(pagers.PageSize).ToList();
