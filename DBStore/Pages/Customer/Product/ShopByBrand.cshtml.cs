...
{
    public class ShopByBrandModel : PageModel
    {
        private IUnitOfWorkRepository productRepository;

        public ShopByBrandModel(IUnitOfWorkRepository productRepository)
        {
        ...
        public IActionResult OnGet(int id, int catid)
        {
            ...
            int resCount = productRepository.Product.GetAll().Where(s => s.BrandId == catid).Count();
            var pagers = new PagedList(resCount, id, pageSize);
            int recSkip = (id - 1) * pageSize;
            var data = productRepository.Product.GetAll().Where(s => s.BrandId == catid).Skip(recSkip).Take(pagers.PageSize).ToList();
